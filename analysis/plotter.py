...
def create_ID_card(folder_list,
    ...
    for folder in folder_list : 
        ...
        for cluster_folder in clusters_folders :
            ...
            make_pg2(cluster_folder = cluster_path, end_TC = end_TC, fs = fs)
            pbar.update(1)

            make_pg2_gratings(cluster_folder = cluster_path, end_TC = end_TC, fs = fs)
            pbar.update(1)
            ...
            for i in np.arange(5, 13, 1) :
                make_PSTH_pages(cluster_folder = cluster_path,
                             n = i, plot_type = 'PSTH',
                             beg_PST = beg_PST, end_PST = end_PST,
            ...
            for i in np.arange(13, 21, 1) :
                make_grat_PSTH_pages(cluster_folder = cluster_path,
                             n = i, plot_type = 'PSTH',
                             beg_PST = beg_PST, end_PST = end_PST,
                             win_size = win_size, step_size = step_size,
                             binsize = binsize)
            ...
            pdfs = [cluster_path + '/tmp%s.pdf' % x for x in np.arange(1,20)]
...
def make_pg2(cluster_folder, end_TC, fs):
    '''
...
def make_pg2_gratings(cluster_folder, end_TC, fs):
    '''
    Creates the final page, with merged and non merged tuning curve, as well
    as the neurometric curve
    '''
    ...
    unique_bthetas = np.load(cluster_folder + '/unique_sfs.npy')
    ...
    # Merged TC plot
    mean_FR_per_theta = np.load(cluster_folder + '/plot_grat_TC_merged_means.npy')
    stds_FR_per_theta = np.load(cluster_folder + '/plot_grat_TC_merged_stds.npy')
    r_squared = np.load(cluster_folder + '/plot_neurometric_grat_fit_reports.npy')
    
    fitted_merge_curve = np.load(cluster_folder + '/plot_neurometric_grat_merged_fitted_TC.npy')
    btheta_merge_curve = np.load(cluster_folder + '/plot_neurometric_grat_merged_Btheta_fits.npy')
    r_squared_merge_curve = np.load(cluster_folder + '/plot_neurometric_grat_merged_fit_reports.npy')

    # Non merged TC
    mean_FR_per_btheta = np.load(cluster_folder + '/plot_grat_TC_nonmerged_means.npy')
    stds_FR_per_btheta = np.load(cluster_folder + '/plot_grat_TC_nonmerged_stds.npy')
    fitted_TC = np.load(cluster_folder + '/plot_neurometric_grat_fitted_TC.npy')

    max_FR = np.max(mean_FR_per_btheta)*1.1
    min_FR = np.min(mean_FR_per_btheta)*.9

    
    plt.text(0.38, .72, 'GRATINGS GRATINGS - Spikes on [0;%.1f]s interval'% end_TC, fontsize = 12, transform = plt.gcf().transFigure)
...
#                  capsize = 2,c = 'gray')
    axs1.set_xlabel(r'$\theta$' + '°')
    axs1.set_ylabel('FR (sp/s)')
    axs1.set_title('Tuning curve, averaged over all ' + 'sf' +'\n' + r'$B_\theta$fit = %.2f' % btheta_merge_curve[0] + '    r² = %.2f' % r_squared_merge_curve[0],
     fontsize = 10)
    axs1.set_ylim(min_FR, max_FR)
    
    # Neurometric TC plot
    bthetas_fit = np.load(cluster_folder + '/plot_neurometric_grat_Btheta_fits.npy')
    axs2.plot(unique_bthetas, bthetas_fit, 'k')
    axs2.yaxis.tick_right()
    axs2.yaxis.set_label_position('right')
    axs2.set_xlabel(r'$B_\theta$' + ' stim')
    axs2.set_ylabel(r'$B_\theta$' + ' fit')
    axs2.set_title('TC opening for each ' +'sf', fontsize = 10)
...
#                  capsize = 2,c = 'gray')
        ...
        if i == 0 or i == 4:
            ax.set_title('Tuning curve for one sf', fontsize = 10)
        
        if i <= 3 : 
            plt.text(0.38, ys[i]+.015, 'sf' + '=%.1f°' % unique_bthetas[i], fontsize = 12, transform = plt.gcf().transFigure)
            plt.text(0.38, ys[i]-.02, r'$B_\theta$ fit' + '=%.1f°' % bthetas_fit[i], fontsize = 12, transform = plt.gcf().transFigure)
            plt.text(0.38, ys[i]-.035, 'r² = %.2f' % r_squared[i], fontsize = 12, transform = plt.gcf().transFigure)
        else : 
            plt.text(0.78, ys[i-4]+.015, 'sf' + '=%.1f°' % unique_bthetas[i], fontsize = 12, transform = plt.gcf().transFigure)
            plt.text(0.78, ys[i-4]-.02, r'$B_\theta$ fit' + '=%.1f°' % bthetas_fit[i], fontsize = 12, transform = plt.gcf().transFigure)
            plt.text(0.78, ys[i-4]-.035, 'r² = %.2f' % r_squared[i], fontsize = 12, transform = plt.gcf().transFigure)
...
def make_pg3(cluster_folder, step_size, win_size,
    ...
    fig.savefig(cluster_folder + '/tmp4.pdf', bbox_inches = 'tight')
    plt.close(fig)
...
def make_PSTH_pages(cluster_folder,
                 beg_PST, end_PST,
                 win_size, step_size,
                 binsize,
                 n, plot_type):
    ...
    i = np.where(np.arange(5, 13, 1) == n)[0][0] #get b_theta nbr
...
def make_grat_PSTH_pages(cluster_folder,
                 beg_PST, end_PST,
                 win_size, step_size,
                 binsize,
                 n, plot_type):
    ...
    i = np.where(np.arange(13, 21, 1) == n)[0][0] #get b_theta nbr

    PSTH_list = np.load(cluster_folder + '/plot_grat_PSTH_nonmerged.npy', allow_pickle = True)[i]
    ...
    plt.suptitle('SF' + ' = %.3f°' % unique_bthetas[i] + '\nHistogram with %sms bin size' % binsize,
                 y = .95, fontsize = 15 )
